...
const Videos = () => {
    ...
    const handleOnClick = (item, index) => {
        setVideoSrc(item);
        setSampleCount(index + 1);
    }
  return (
    <>
      {/* <hr style={{ backgroundColor: 'black', opacity: '0.7',marginTop:'70px', marginBottom:'10px'}} /> */}
      <Stack direction={['column', 'row']} h={'100vh'} mt={16}>
        <VStack w={'full'}>
          ...
          <VStack alignItems={'flex-start'} p={8} w={'full'} overflowY={'auto'}>
            <Heading>Video {sampleCount}</Heading>
            <Text>
              This is a sample video {sampleCount} for testing and demo.
            </Text>
          </VStack>
        </VStack>
